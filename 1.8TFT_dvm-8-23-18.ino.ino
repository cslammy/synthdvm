...
//DC mode now works.
//adding AC-RMS by throwing a switch

//current software as of 7:36 Thurs AUG 24TH
...
int RMSpin = A0; // read output of AC to RMS conversion here.
...
float sensorValue = 0;
//float multipier = .012
float multiplier = .01188;
float multiplierneg = .012;
...
//pin to read AC or DC mode is D10
pinMode(ACDCPin,INPUT);
...
{
ACDC=digitalRead(ACDCPin);

if (ACDC == LOW)
// switch on front set to read DC--Read DC voltage and display to screen
// Get sensor values for VU meter for DC plus and minus V
{
    ...
    if (polarityValue > 0)
      ...
      pinMode(3,OUTPUT);
      digitalWrite(3,LOW); // send to ground; clobber inverting opamp
    ...
    if (polarityValue == 0)
      ...
      pinMode(12,OUTPUT);
      digitalWrite(12,LOW); //clobber non inverting opamp output
      // Serial.println("you are polarity false"); 
      //  Serial.println(polarityValue); 
      negValue = analogRead(negPin);
      displayValue2 = (negValue * multiplierneg);
      }  
     else
      {
      pinMode(3,OUTPUT);
    ...
    //write DC voltage to screen

// get rid of RMS from AC reading
    ...
    tft.drawChar(102,30,'d',0xFFF,0x000,2);
    tft.drawChar(114,30,'c',0xFFF,0x000,2);
...
//Clobber DC OpAmps
      ...
      pinMode(12,OUTPUT);  
      pinMode(3,OUTPUT);
      digitalWrite(12,LOW); //clobber non inv input
      digitalWrite(3,LOW); //clobber inv input
      delay(300);
      digitalWrite(LED_BUILTIN, LOW);
      ...
      tft.setTextColor(0xFFFF,0x0000);
       ...
       //get rid of DC
    tft.drawChar(102,30,'d',0x000,0x000,2);
    tft.drawChar(114,30,'c',0x000,0x000,2);

// get rid of minus symbol
